...
//获取树状区域
  $.ajax({
	...
	ajaxs(url_main+"isp/api/v1/protected/shopSelect","get","",function(res2){
    	...
    	var initStore = [{shopId:"",shopName:"请选择小店"}]
	...
	$.ajax({
		...
		success:function(res){
			if(res.success==true&&res.data){
    			...
    			$("#isStoreMan").prop("checked",formobjs.isShopManager== 1?true:false)
...
.on("blur","#phone",function(){
	...
	if(!$("#isStoreMan:checked").val()){
		formobjs.isShopManager="0"
	}else{
		formobjs.isShopManager="1"
	}
...
function checkForms(formobjs){
	...
	var checck = region("#editVal","#storeId","#shopId");
